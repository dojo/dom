...
setting styles directly inline on one element. When used, the `addCssRule` returns a handle that allows the rules for the provided selector to be manipulated.
...
as efficiently as possible, the `schedule` package provides functionality to group the operations together.
